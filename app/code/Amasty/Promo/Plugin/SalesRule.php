...
class SalesRule
{
    ...
    public function afterLoadPost(\Magento\SalesRule\Model\Rule $subject, $salesRule)
    {
        ...
        foreach (GiftRuleInterface::IMAGE_FIELDS as $field) {
            if (!$amRule->hasData($field)) {
                $amRule->setData($field, null);
            } else {
                $image = $amRule->getDataByKey($field);
                $amRule->setData($field, $this->serializer->serialize($image));
            }
        }
