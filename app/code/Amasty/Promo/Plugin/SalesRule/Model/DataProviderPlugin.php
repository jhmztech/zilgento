        ...
        if (is_array($result)) {
            foreach ($result as &$item) {
                if (isset($item[GiftRuleInterface::EXTENSION_ATTRIBUTES_KEY][GiftRuleInterface::EXTENSION_CODE])
                    ...
                    foreach (GiftRuleInterface::IMAGE_FIELDS as $imageField) {
                        if (isset($rule[$imageField]) && is_string($rule[$imageField])) {
                            $rule[$imageField] = $this->serializer->unserialize($rule[$imageField]);
                        }
                    }
